...
First, we derive the critical values at the interim and final analyses. For the interim analysis, only one log-rank test will be used, hence the critical value is $u_1 = \Phi^{-1}(1-0.0015) = 2.968$. For the final analysis, the critical value $u_2$ satisfies 
\[
...
\[
  P_0(W_{1,1} < u_1, W_{2,1} < u_2, W_{2,2} < u_2) = 0.975 \; 
    (Eq. 1)
\]
Let $U_{i, FH(p,q)}$ denote the numerator of the FH(p,q) weighted log-rank test statistic at analysis $i$, and $V_{i,FH(p,q)}$ its variance. Then $W_{i, FH(p,q)} = U_{i, FH(p,q))}/{V_{i,FH(p,q)}^{1/2}}$. In addition, we can show that \[
...
```
It follows that the mean of $\mathbf{W}=(W_{1,1}, W_{2,1}, W_{2,2})$ is
\[
