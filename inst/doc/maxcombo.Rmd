...
$$ which is the same as $$
  P_0(W_{1,1} < u_1, W_{2,1} < u_2, W_{2,2} < u_2) = 0.975 \; 
    (Eq. 1)
$$ Let $U_{i, FH(p,q)}$ denote the numerator of the FH(p,q) weighted
log-rank test statistic at analysis $i$, and $V_{i,FH(p,q)}$ its
variance. Then $W_{i, FH(p,q)} = U_{i, FH(p,q))}/{V_{i,FH(p,q)}^{1/2}}$.
...
For the simulation study, we use infinite critical values for the
